...
// ── Tree-sitter parser ────────────────────────────────────────────────────────
function extractNodeText(node, source) {
...
function parseNodeSymbols(node, source, symbols, depth = 0) {
...
function extractImportsFromTree(rootNode, source) {
    const imports = [];
    function walk(node) {
...
function parseWithTreeSitter(filePath, source) {
...
function parseWithRegex(filePath, source) {
    ...
    while (i < lines.length) {
        ...
        if (impMatch) {
            ...
            if (namedStr)
                names.push(...namedStr.split(',').map(s => s.trim()).filter(Boolean));
            imports.push({
                source: src,
                names,
