export type CognitiveEventType = 'goal:created' | 'plan:generated' | 'step:executed' | 'runtime:validated' | 'learning:observed' | 'recovery:triggered' | 'rollback:applied' | 'constraint:violated' | 'mode:switched';
interface BaseEvent {
    readonly id: string;
...
export type CognitiveEvent = GoalCreatedEvent | PlanGeneratedEvent | StepExecutedEvent | RuntimeValidatedEvent | LearningObservedEvent | RecoveryTriggeredEvent | RollbackAppliedEvent | ConstraintViolationEvent | ModeSwitchedEvent;
export declare function makeEventId(): string;
export declare function createEvent<T extends CognitiveEvent>(partial: Omit<T, 'id' | 'timestamp'> & {
    id?: string;
    timestamp?: Date;
}): T;
export {};
//# sourceMappingURL=CognitiveEvent.d.ts.map