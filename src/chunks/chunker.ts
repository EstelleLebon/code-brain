...
import { SymbolNode, SemanticChunk, TruthLevel } from '../types/index.js';

function makeChunkId(symbolId: string): string {
  return createHash('sha256').update('chunk:' + symbolId).digest('hex').slice(0, 16);
}

function buildSummary(sym: SymbolNode, _content: string): string {
  const parts: string[] = [];
  ...
  if (sym.kind === 'class') {
    ...
    if (extendsMatch?.[1]) parts.push(`extends: ${extendsMatch[1]}`);
    if (implementsMatch?.[1]) parts.push(`implements: ${implementsMatch[1].trim()}`);
  }
...
export function chunkSymbol(sym: SymbolNode, rawSource?: string): SemanticChunk {
...
export function chunkSymbols(
  symbols: SymbolNode[],
  sourceMap?: Map<string, string>  // symbolId -> raw source
): SemanticChunk[] {
