...
import os from 'os';
import { CodeBrain } from '../api/api.js';

async function main(): Promise<void> {
  const repoPath = process.argv[2] ?? process.cwd();
  const dbPath = path.join(os.tmpdir(), 'code-brain-example.db');
  ...
  for (const result of relevant) {
    ...
    console.log(`    File: ${path.relative(repoPath, result.trace.source)}`);
    if (chunk.summary) {
  ...
  if (ctx.symbols.length > 0) {
    ...
    for (const sym of ctx.symbols.slice(0, 3)) {
      console.log(`  • [${sym.kind}] ${sym.name} — ${path.relative(repoPath, sym.filePath)}:${sym.startLine}`);
    }
  }
  ...
  brain.onWatchEvent((evt) => {
    console.log(`  [watch] ${evt.event}: ${path.basename(evt.filePath)} (changed: ${evt.changed})`);
  });
